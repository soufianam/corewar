...
/*   Updated: 2018/04/16 18:53:13 by pprikazs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		cw_check_reg_legality(int i, t_instruct *inst)
{
	int		ret;
...
static int		cw_check_label_legality(int i, char *param, t_instruct *inst)
{
	int		ret;
...
extern int		cw_insert_param(char *param, t_instruct **inst, int i)
{
	...
	if (param == 0)
		return (ret);
	if (ret != 0 && param[0] == 'r')
		ret = cw_check_reg_legality(i, *inst);
	else if (ret != 0 && param[0] == DIRECT_CHAR)
		ret = cw_check_label_legality(i, param, *inst);
	else if (ret != 0)
