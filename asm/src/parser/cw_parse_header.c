...
/*   Updated: 2018/04/19 17:18:53 by pprikazs         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char			ft_getstr_head(char *line, char **buff)
{
	...
	if (*buff == 0)
	{
		while (line[start] != '"' && line[start] != '\0')
			start++;
		if (line[start] == '\0')
			return (-1); //flag .name ou .comment mais pas de définition
	}
...
extern int		cw_parse_header(char *line, header_t *head)
{
	...
	ret = 1;
	if (ft_strncmp(NAME_CMD_STRING, line, \
				ft_strlen(NAME_CMD_STRING)) == 0 || stat == 1)
		ret = cw_parse_header_checkname(line, head, &buff, &stat);
	else if (ft_strncmp(COMMENT_CMD_STRING, line, \
				ft_strlen(COMMENT_CMD_STRING)) == 0 || stat == 2)
		ret = cw_parse_header_checkcomment(line, head, &buff, &stat);
	if (ret == 1 && (*(head->prog_name) == '\0' || *(head->prog_name) == '\0'))
		ret = -1; //Pas de .comment ou pas de .name
	return (ret);
}
