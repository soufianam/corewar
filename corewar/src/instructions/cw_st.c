...
/*   Updated: 2018/04/20 12:00:35 by blefeuvr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		cw_st(t_vm *vm, t_process *process)
{
	...
	{
		if (ret[1] == 1)
			ft_memcpy(process->registries[param[1] - 1], process->registries[param[0] - 1], REG_SIZE);
		else
			ft_memcpy(&(vm->vm[(process->pc + process->entrypoint + ((short)param[1] % IDX_MOD) - ret[1] - 2) % MEM_SIZE]), process->registries[param[0] - 1], REG_SIZE);
